...
export class Renderer {
  ...
  public async init(canvas: HTMLCanvasElement, numLights: number): Promise<boolean> {
    ...
    materialDataBuffer = CreateUniformBuffer(device, 4000)
...
export class Renderer {
  ...
  private presentationFormat: GPUTextureFormat
  private presentationSize: number[]
  private transparentNodes: [ObjMesh, number][] = []
  ...
  public makeNodes(node: Node3d, nodes: Node3d[], cam: Float32Array) {
    if (node instanceof ObjMesh) {
      if (node.getMaterial().getIsTransparent()) {
        let camPos = vec3.fromValues(cam[0], cam[1], cam[2])
        let nodePos = vec3.fromValues(node.x, node.y, node.z)
        let distanceVec = vec3.create()
        vec3.subtract(distanceVec, camPos, nodePos)
        let distance = vec3.length(distanceVec)
        this.transparentNodes.push([node, distance])
      } else {
        nodes.push(node)
      }
    }
    for (const child of node.children) {
      this.makeNodes(child, nodes, cam)
    }
  }
  ...
  public frame(camera: Camera, light: Light, node: Node3d) {
    ...
    const passEncoder = commandEncoder.beginRenderPass(this.renderPassDescriptor)
    const nodes: Node3d[] = []
    this.makeNodes(node, nodes, camera.getCameraEye())
    if (this.transparentNodes.length > 0) {
      this.transparentNodes.sort(function ([x, a], [y, b]) {
        return b - a
      })
      for (const obj of this.transparentNodes) {
        nodes.push(obj[0])
      }
    }
