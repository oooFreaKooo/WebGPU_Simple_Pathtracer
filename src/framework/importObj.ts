...
export async function parseOBJ(device: GPUDevice, file: string) {
  ...
  const _uvData = new Float32Array(output.models[0].textureCoords.length * 2)
  let offset = 0
  ...
  _material.setLight()
  await _material.setTexture("../src/examples/obj/despacitospidertx.png")

  return new Object3d(device, _vertices, _normals, _indices, _material, _uvData)
}
